...
ScriptingModule::ScriptingModule() : Module("ScriptingModule") {
  sol_state_["LogE"] = [](const std::string& message) {
    LogE("{}", message);
  };
  sol_state_["LogW"] = [](const std::string& message) {
    LogW("{}", message);
  };
  sol_state_["LogI"] = [](const std::string& message) {
    LogI("{}", message);
  };
  sol_state_["LogD"] = [](const std::string& message) {
    LogD("{}", message);
  };
  sol_state_["LogV"] = [](const std::string& message) {
    LogV("{}", message);
  };
