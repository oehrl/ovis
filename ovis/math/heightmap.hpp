...
template <typename T>
inline std::vector<glm::vec3> GenerateHeightmapVertices(array_view<T> values,
                                                        std::size_t width,
                                                        std::size_t depth) {
  ...
  for (std::size_t z = 0; z < depth_minus_one; ++z) {
    for (std::size_t x = 0; x < width_minus_one; ++x) {
      vertices.emplace_back(x + x_offset,
                            static_cast<float>(values[y * width + x]),
                            z + z_offset);
    }
  }
  return vertices;
...
template <typename T>
inline std::vector<T> GenerateHeightfieldIndices(std::size_t width,
                                                 std::size_t depth) {
