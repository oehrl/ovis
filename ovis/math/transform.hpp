...
class Transform {
 public:
  ...
  inline void SetTranslation(const glm::vec3& new_translation) {
  ...
  inline void Rotate(glm::quat rotation_offset) {
    rotation_ = rotation_offset * rotation_;
  }
  inline void Rotate(glm::vec3 axis, float angle_in_radians) {
    Rotate(glm::angleAxis(angle_in_radians, axis));
  }
  inline void SetYawPitchRoll(float yaw, float pitch, float roll) {
    rotation_ = glm::angleAxis(yaw, glm::vec3{0.0f, 1.0f, 0.0f}) *
