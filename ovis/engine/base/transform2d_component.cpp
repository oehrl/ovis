...
SceneObjectComponent::PropertyType Transform2DComponent::GetPropertyType(
...
SceneObjectComponent::PropertyValue Transform2DComponent::GetProperty(
    const std::string& property_name) const {
  if (property_name == "Position") {
...
void Transform2DComponent::SetProperty(const std::string& property_name,
  ...
  if (property_name == "Position") {
    ...
    transform_.SetRotation(
        glm::quat(glm::vec3(0.0f, 0.0f, std::get<float>(value))));
  } else if (property_name == "Scale") {
    SDL_assert(std::holds_alternative<glm::vec2>(value));
